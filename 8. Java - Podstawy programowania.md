...
>[!NOTE]
> ℹ️ <ins>Należy na początku pliku dodać import: `import java.until.*`, który importuje potrzebne nam obiekty `Map` oraz `List`.</ins>
>
...
1. Utwórz klasę `Course`
   - Pola które posiada: `String title`, `Teacher teacher`, `List<Student> students`, `List<Assigments> assigments`.
   - Metody które potrzebuje posiadać klasa:
     - `addStudent(Student student)` -  dodaje studenta do listy uczestników,
     - `addAssigment(Assignment assignment)` - dodaje zadanie do listy zadań,
     - `listAssigments()` - wypisuje wszystkie zadania przypisane do kursu,
     - `listStudents()` - wypisuje wszystkich studentów przypisanych do kursu.
5. Utwórz klasę `Assigment`
   - Pola do zainicjowania są następujące: `String title`, `String description`,`Map<Student, String> submissions`,`Map<Student, int> grades`.
...
- Marek zapisuje się na kurs: `marek.enrollInCourse(javaCourse)`
